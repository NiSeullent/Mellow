...
//
// Mellow register access:
//   MellowCore::callback->readReg32(addr)
//   MellowCore::callback->writeReg32(addr, val)
//   MellowCore::callback->intel_de_rmw(addr, clear_mask, set_bits)

#include "IntelDPLinkTraining.hpp"
#include "kern_mellow.hpp"          // MellowCore::callback, readReg32/writeReg32/intel_de_rmw
#include <Headers/kern_util.hpp>   // SYSLOG

// ─── helpers ─────────────────────────────────────────────────────────────────

static uint32_t ng_read(uint32_t reg) {
    return MellowCore::callback->readReg32(reg);
}
static void ng_write(uint32_t reg, uint32_t val) {
    MellowCore::callback->writeReg32(reg, val);
}
static void ng_rmw(uint32_t reg, uint32_t clear, uint32_t set) {
    MellowCore::callback->intel_de_rmw(reg, clear, set);
}
    ...
    const uint8_t preEmphasis[4])
{
    if (!MellowCore::callback) {
        SYSLOG("mellow", "IntelDPLinkTraining: callback is null");
        return;
    }
    if (phy > 4) {
        SYSLOG("mellow", "IntelDPLinkTraining: PHY %u out of range (0=A,1=B,2=C,3=D,4=E)", phy);
        return;
    }
    ...
    SYSLOG("mellow", "IntelDPLinkTraining: PHY %u programmed (HBR2=%d lanes=%u)", phy, isHBR2, laneCount);
}
...
{
    if (!MellowCore::callback) {
        SYSLOG("mellow", "IntelDPLinkTraining ADLP: callback is null");
        return;
    }
    // ADL-P combo PHY: only ports A (0) and B (1) — TC ports C-F are DKL PHY
    if (phy > 1) {
        SYSLOG("mellow", "IntelDPLinkTraining ADLP: PHY %u is not a combo PHY on ADL-P", phy);
        return;
    }
    ...
    SYSLOG("mellow", "IntelDPLinkTraining ADLP: PHY %u programmed (HBR2=%d eDP=%d lanes=%u)",
